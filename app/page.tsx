...
export default function Home() {
  ...
  const nextFromProfile = () => setStep(mode === "new" ? "checkout" : "profile");
  const setAnswer = (idx: number, value: string) => setAnswers((a) => ({ ...a, [idx]: value }));

  return (
    <main>
      <nav className="nav"><button className="brand" onClick={() => setStep("landing")}><span>너를</span> 알아가는 과정</button>{step === "landing" && <div className="flow-nav" aria-label="페이지 바로가기"><a href="#main-flow"><b>01</b> 메인 플로우</a><a href="#how-it-works"><b>02</b> 사용방법</a><a href="#choose-test"><b>03</b> 선택지</a></div>}<div className="nav-right"><span className="price">지금은 무료</span><span className="secure">둘만의 커플 테스트</span></div></nav>

      {step === "landing" && <>
        <section className="hero snap-section" id="main-flow">
        ...
        <section className="how snap-section" id="how-it-works"><p className="section-step">02. 사용방법</p><div><small>HOW IT WORKS</small><h2>둘이 따로 답하고,<br/>결과는 함께 받아요</h2></div>{[["01","코드 만들기","대표 한 명이 이메일을 입력하고 커플 코드를 만들어요."],["02","각자 답하기","같은 50문항에 서로 상의하지 않고 솔직하게 답해요."],["03","결과 받아보기","맞다·비슷하다·다르다로 비교한 결과지를 이메일로 받아요."]].map(x=><article key={x[0]}><b>{x[0]}</b><h3>{x[1]}</h3><p>{x[2]}</p></article>)}</section>
        <section className="levels snap-section" id="choose-test">
